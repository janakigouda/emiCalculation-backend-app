...
userRoute.post("/calculateEMI",authMiddleWare,(req,res)=>{
    let {loanAmount,annualInterest,tenureMonth}=req.body;
    console.log({loanAmount,annualInterest,tenureMonth});
    try{
        let r= +annualInterest/12/100;
        let E= (+loanAmount) * r *((1+r)** (+tenureMonth)) / ((1+r)** (+tenureMonth)-1);
